...
# Purpose: To toggle various UV Grid Spans features on all visible UV viewports.
...
for w in range(lx.eval("query view3dservice view.N ?")):
    if lx.eval("query view3dservice view.type ? %s" %w) == "UV2D":
        ...
        lx.eval("viewuv.uLowSpan "+MyArg1+"")
        lx.eval("viewuv.uHighSpan "+MyArg2+"")
        lx.eval("viewuv.vLowSpan "+MyArg3+"")
        lx.eval("viewuv.vHighSpan "+MyArg4+"")