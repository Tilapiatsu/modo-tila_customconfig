...
#-------------------------------------------------------------------------------
# Name:			pp_etr_invert_weight.py
# Version:		1.1
# Purpose:		This script is designed to invert the values of a weightmap
#
# Author:		William Vaughan, pushingpoints.com
#               Small changes by Cristobal Vila, etereaestudios.com
#				(more comments and automatic name adding 'inv' to final Weight)
#
# Created:		January 3, 2014 / Modified: February 12, 2014
...
# Get maps that remains selected (only Weight Maps here, since other types have been deselected)
remain_selected_vmaps = lx.evalN('query layerservice vmaps ? selected')
...
# Abort script with a message if less or more than 1 Weight Map is selected
if vmapsN <= 0:
...
# Get name of vmap that remains selected (only 1 Weight, if we arrived here)
vmap_name = lx.eval1('query layerservice vmap.name ? %s' %remain_selected_vmaps)
lx.out('vmap name', vmap_name)
...
lx.eval('vertMap.name %s_inv wght active' % vmap_name)
