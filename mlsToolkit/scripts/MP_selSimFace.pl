...
my $selcheck1 = lxq("query layerservice poly.n ? selected");
lx("!!select.3DElementUnderMouse remove");
my $selcheck2 = lxq("query layerservice poly.n ? selected");
lx("!!select.3DElementUnderMouse add");
my $selcheck3 = lxq("query layerservice poly.n ? selected");
...
if ($selcheck1 == $selcheck2){
	...
	elsif($layernum == 1 and $selcount != 0){
		lx("select.useSet keepsel1 select");
		lx("select.deleteSet keepsel1 delete");		
	}
}
