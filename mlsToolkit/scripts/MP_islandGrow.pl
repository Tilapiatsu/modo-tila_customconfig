...
if(lxq( "!!select.typeFrom {polygon;vertex;edge;item} ?" ) and $viewcheck ne "UV2D"){
	...
	lx("select.polygonConnect uv");
		...
		lx("!!select.editSet islandtemp3 add")if ($islandsel1 == $islandsel2);
	lx("select.expand")if ($islandsel1 == $islandsel3);
	lx("select.contract")if ($islandsel1 == $islandsel2);
		...
		if ($islandsel1 == $islandsel2){
			...
			lx("!!select.useSet islandtemp3 select")if ($polycount3 != 0);
